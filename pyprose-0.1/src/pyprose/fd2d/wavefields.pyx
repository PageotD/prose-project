...
cpdef evolution(np.ndarray[DTYPE_f, ndim=2] mu, np.ndarray[DTYPE_f, ndim=2] lbd, np.ndarray[DTYPE_f, ndim=2] lbdmu, np.ndarray[DTYPE_f, ndim=2] bux, np.ndarray[DTYPE_f, ndim=2] buz, np.ndarray[DTYPE_f, ndim=2] pmlx0, np.ndarray[DTYPE_f, ndim=2] pmlx1, np.ndarray[DTYPE_f, ndim=2] pmlz0, np.ndarray[DTYPE_f, ndim=2] pmlz1, int npml, int isurf, int srctype, np.ndarray[DTYPE_f, ndim=1] tsrc, np.ndarray[DTYPE_f, ndim=2] gsrc, np.ndarray[np.int16_t, ndim=2] acqui, DTYPE_f dh, int nt, DTYPE_f dt):
    ...
    cdef np.ndarray[DTYPE_f, ndim=2] wavex = np.zeros((nt, nrec), dtype=np.float32)
    cdef np.ndarray[DTYPE_f, ndim=2] wavez = np.zeros((nt, nrec), dtype=np.float32)

    cdef np.ndarray[DTYPE_f, ndim=2] pmlx0p = 1./dt + pmlx0[:,:]
    cdef np.ndarray[DTYPE_f, ndim=2] pmlx0m = 1./dt - pmlx0[:,:]
    cdef np.ndarray[DTYPE_f, ndim=2] pmlx1p = 1./dt + pmlx1[:,:]
    cdef np.ndarray[DTYPE_f, ndim=2] pmlx1m = 1./dt - pmlx1[:,:]
    cdef np.ndarray[DTYPE_f, ndim=2] pmlz0p = 1./dt + pmlz0[:,:]
    cdef np.ndarray[DTYPE_f, ndim=2] pmlz0m = 1./dt - pmlz0[:,:]
    cdef np.ndarray[DTYPE_f, ndim=2] pmlz1p = 1./dt + pmlz1[:,:]
    cdef np.ndarray[DTYPE_f, ndim=2] pmlz1m = 1./dt - pmlz1[:,:]

    # Loop over time samples
    for it in range(0, nt):
        #print(it+1, nt, np.amax(np.abs(ux)), np.amax(np.abs(uz)))
        ...
        for irec in range(nrec):
            wavex[it, irec] = ux[acqui[irec, 1], acqui[irec, 0]]
            wavez[it, irec] = uz[acqui[irec, 1], acqui[irec, 0]]

    return ux, uz, wavex, wavez